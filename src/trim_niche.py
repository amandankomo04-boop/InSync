...
def flatten_and_trim_niches(train_limit=3000):
    # ABSOLUTE PATH - Double check this matches your Desktop exactly
    ...
    # Get categories (Fashion, Travel, etc.)
    ...
    for niche in niches:
        ...
        if not all_found_images:
            print(f"  ⚠️ No images found in {niche}. Skipping.")
            continue

        # 2. Select the 3,000 images
        if len(all_found_images) > train_limit:
            ...
            print(f"  ℹ️ {niche} only has {len(all_found_images)} images. Using all.")
        ...
        # 4. Copy selected images to holding area (FLATTENING)
        ...
        # 5. CLEAR OUT the original niche folder (Deletes all subfolders like 'men', 'women')
